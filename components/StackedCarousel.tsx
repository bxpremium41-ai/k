    ...
    <div className="w-full bg-white text-gray-900 pt-12 pb-24 md:py-24 overflow-hidden relative">
        ...
        <div className="container mx-auto px-4 md:px-12 flex flex-col lg:flex-row items-center justify-between gap-0 lg:gap-16 relative z-10">
            ...
            <div className="flex-1 w-full lg:max-w-xl flex flex-col items-center lg:items-start text-center lg:text-left order-1 mb-16 lg:mb-0">
                ...
                <h2 className="text-4xl md:text-6xl font-display font-bold leading-[1.1] mb-8 text-gray-900 tracking-tighter">
                    Skills that will <br/>
                    <span className="animate-skyrocket mx-1">Skyrocket</span> 
                    <TrendingUp className="inline-block mb-2 text-brand-primary ml-2" size={40} />
                    <br/>
                    your Studio Income.
                </h2>
                ...
                <div className="h-[100px] relative w-full mb-10">
                     {COURSES.map((course, idx) => {
                         ...
                         <div 
                            key={course.id} 
                            className={`absolute top-0 left-0 w-full flex flex-col items-center lg:items-start transition-all duration-700 ${
                                isActive 
                                ? 'opacity-100 translate-y-0 z-10' 
                                : 'opacity-0 translate-y-8 pointer-events-none z-0'
                            }`}
                         >
                            <p className="text-gray-500 text-lg leading-relaxed max-w-md mb-6 mx-auto lg:mx-0 font-medium">
                                {course.description}
                            </p>
                         </div>
                     )})}
                    ...
                    {COURSES.map((_, idx) => (
                        <div 
                            ...
                            className={`h-1 cursor-pointer transition-all duration-500 rounded-full ${
                                idx === activeIndex ? 'w-16 bg-brand-primary shadow-lg shadow-brand-primary/20' : 'w-4 bg-gray-100 hover:bg-gray-200'
                            }`}
                        />
                    ))}
                </div>
                     ...
                     return (
                         ...
                         >
                            <div className="mb-8 text-center">
                                <div className="text-brand-primary text-[10px] font-black uppercase tracking-[0.3em] mb-3 bg-brand-primary/10 border border-brand-primary/20 px-4 py-1.5 rounded-full inline-block">
                                    COURSE {String(idx + 1).padStart(2, '0')} • {course.software}
                                </div>
                                <h3 className="text-3xl sm:text-4xl font-display font-bold text-gray-900 tracking-tighter">
                                    {course.title}
                                </h3>
                            </div>

                            <div className={`${frameSize} rounded-[2.5rem] overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.1)] border border-gray-100 bg-gray-50 relative group cursor-pointer`} onClick={() => onCourseClick(course)}>
                                ...
                                <div className="absolute bottom-8 left-0 w-full flex justify-center scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all">
                                    <button className="bg-brand-primary text-white px-8 py-3 rounded-xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-brand-primary/20">
